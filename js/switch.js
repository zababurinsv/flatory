(function($){
  $.iphoneStyle = {
    defaults: { checkedLabel: 'ON', uncheckedLabel: 'OFF', background: '#fff', width: '' }
  }
  ...
  $.fn.iphoneStyle = function(options) {
    ...
    return this.each(function() {
      ...
      elem.css({ opacity: 0 });
      elem.wrap('<div class="switch_container" ' + ( options.width ? 'style="width: ' + options.width + 'px"' : '' ) + ' />');
      ...
      var handle    = elem.siblings('.handle'),
          ...
          container = elem.parent('.switch_container'),
          rightside = container.width() - 40; //39

      rightside = container.width() ? rightside : 48;

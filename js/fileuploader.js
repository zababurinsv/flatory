...
function FileUploader(ioptions) {
    ...
    if (this.options['uploadscript'] == undefined)
        return null;
            ...
            if (evt.target.readyState == FileReader.DONE) {
                ...
                xhr.upload.addEventListener("progress", function(evt) {
                    if (evt.lengthComputable) {
                        ...
                        // вызываем функцию рендера прогресс бара
                        if (typeof that.options['callback_progress'] === 'function')
                            that.options.callback_progress(percentComplete, that.options);
                    }
...
//                                window.parent.location = that.options['redirect_success'];
                                // callback success
                                var callback_success = that.options['callback_success'];
                                if (typeof callback_success === 'function')
                                    callback_success(evt, that.options);
                            }
    ...
    this.Upload = function() {
        ...
        if (!this.file)
            ...
            if (this.filesize > this.options['portion'])
                this.UploadPortion(0, this.options['portion']);

            // Иначе отправим файл целиком
            else
                this.UploadPortion(0, this.filesize);
        }
    }
...
//            }, false);
        ...
        // файл уже выбран и передается в конфиге file_selected
        var f = this.options['file_selected'];
        that.filesize = f.size;
        that.file = f;
